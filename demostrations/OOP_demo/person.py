...
class Person:
    ...
    def is_mature(age):
        if age >= 18:
            ...
            return "Person is inmature"


    #Inttialiaser/Constructor - function automaticalluy called when object instantiated (magic method)
    #Initiliaser of Any class has the same name
    #'self' - reference to specific object beijng instatiated
    ...
    def __init__(self, name="Maria", age=0, job="none", w=MAX_W):
        ...
        if w > Person.MAX_W:
            ...
            self.weight = w
        #str() magic method -> invoked automaticaly when print() function is called [informal representation]
    def __str__(self):
        return f"{self.name} is {self.age} years old. They work as {self.job} and weight {self.weight} kg"

    #magic method repr provides "formal representation of an object via string. How we wish to store object for later
    def __repr__(self):
    ...
    #Instance Method ->a method that applies to specific instance of the class
    def eat(self, food, w):
